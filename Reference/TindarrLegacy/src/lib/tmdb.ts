...
class TMDBClient {
  ...
  /**
   * Make authenticated request using Read Access Token (recommended method)
   */
  private async requestWithToken<T>(endpoint: string, params?: Record<string, any>): Promise<T> {
    ...
    if (!response.ok) {
      ...
      throw new Error(`TMDB API error: ${response.status} - ${error.status_message || response.statusText}`)
    }
  ...
  async discoverMovies(params: DiscoverMovieParams = {}): Promise<TMDBDiscoverResponse> {
    return this.requestWithToken<TMDBDiscoverResponse>('/discover/movie', params)
  }
