...
interface PreferencesModalProps {
  isOpen: boolean
  onClose: () => void
  /** Called after preferences are saved successfully (e.g. to reload swipe deck). */
  onAfterSave?: () => void
}
...
export default function PreferencesModal({
  ...
  /** Cycle: no preference → preferred → prefer not → no preference */
  const cycleGenre = (genreId: number) => {
    setPrefs((prev) => {
      const preferred = prev.preferredGenres.includes(genreId)
      const excluded = prev.excludedGenres.includes(genreId)
      if (!preferred && !excluded) {
        return { ...prev, preferredGenres: [...prev.preferredGenres, genreId] }
      }
      if (preferred) {
        return {
          ...prev,
          preferredGenres: prev.preferredGenres.filter((id) => id !== genreId),
          excludedGenres: [...prev.excludedGenres, genreId],
        }
      }
      return {
        ...prev,
        excludedGenres: prev.excludedGenres.filter((id) => id !== genreId),
      }
    })
  }
        ...
        <div className="space-y-6">
          <div>
            ...
            <p className="mt-2 text-xs text-gray-400">
              Click to cycle: default → preferred (green) → prefer not (red) → default
            </p>
          </div>
            ...
            <div className="flex gap-4">
              <div className="flex-1">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setPrefs({ ...prefs, minRating: parseFloat(e.target.value) })
                  }
              ...
              <div className="flex-1">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setPrefs({ ...prefs, maxRating: parseFloat(e.target.value) })
                  }
                  className="w-full rounded-lg bg-slate-700 px-3 py-2 text-white"
                />
              </div>
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setPrefs({
                  ...prefs,
                  originalLanguage: e.target.value || undefined,
                })
              }
              className="w-full rounded-lg bg-slate-700 px-3 py-2 text-white"
            >
            ...
            <p className="mt-1 text-xs text-gray-400">
              Filter movies by their original language
            </p>
          </div>
          ...
          <div>
            ...
            <select
              value={prefs.region ?? ''}
              onChange={(e) =>
                setPrefs({ ...prefs, region: e.target.value || undefined })
              }
              className="w-full rounded-lg bg-slate-700 px-3 py-2 text-white"
            >
            ...
            <p className="mt-1 text-xs text-gray-400">
              Filter movies by release country
            </p>
          </div>
        </div>
